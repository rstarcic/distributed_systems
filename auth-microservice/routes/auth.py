...
async def login( 
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{DB_SERVICE_ENDPOINT}/users", params={"email": username}) as db_response:
            ...
            raise HTTPException(status_code=503, detail=f"Error connecting to the database service: {str(e)}")
        except Exception as e:
