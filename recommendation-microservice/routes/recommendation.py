        ...
        async with aiohttp.ClientSession() as session:
            ...
            if response.status == 200:
                ...
                for recipe in recipes:
                    ...
                    if avoid:
                        if any(ingredient.lower() in recipe_ingredients for ingredient in avoid):
                            recipe_passes = False
                    ...
                    if ingredients:
                        if match_all:
                            if not all(query_ingredient.lower() in recipe_ingredients for query_ingredient in ingredients):
                                recipe_passes = False
                        else:
                            if not any(query_ingredient.lower() in recipe_ingredients for query_ingredient in ingredients):
                                recipe_passes = False
                                
                    if max_ingredients and recipe["ingredients_count"] > max_ingredients:
                        recipe_passes = False
...
async def get_random_recipe(token: str = Depends(verify_user_token)):
    headers = {'Authorization': f'Bearer {token}'}
    try:
        async with aiohttp.ClientSession() as session:
            response = await session.get(f"{DATABASE_SERVICE_URL}/recipes", headers=headers)
